...
class SQ: 
	...
	def freq(self,compare=None,
		...
		fig.update_layout(template='plotly_white',height=fheight,width=fwidth,
						 ...
						 margin=dict(l=40, r=40, t=50, b=10));fig.show()
	...
	''' Computes all possible proteins in an amino acid sequence in RF '''
	...
	def find_pattern(self,pattern,  #  sequence of interest
		...
		if(find_id is 'first'):
			...
			if (re_search != None):
				print('showing first for {pattern}')
			...
			re_search = finditer(pat,self.seq)
			result = []
			for x in re_search:
	...
	def divide_loc(enzyme, sequence):
		...
		def iubrex(IUB):   
			...
			dic = {"A":"A", "C":"C", "G":"G", "T":"T",  # main 4 bases
					# Additional Cases
					"R":"[GA]", "Y":"[CT]", "M":"[AC]", # purine, pyrimidine, amino
					"K":"[GT]", "S":"[GC]", "W": "[AT]", # keto, strong, weak
					"B":"[CGT]", "D":"[AGT]", "H":"[ACT]",  # not A, not C, not G, not T
					"V":"[ACG]", "N":"[ACGT]"} #  not T, any 
